...
def read_fort14(filename: os.PathLike):
    ...
    with open(filename, "r") as file:
        ...
        with StringIO(
            "\n".join(file.readline() for _ in range(num_nodes))
        ) as nodes_stream:
        ...
        with StringIO(
            ...
            output["elements"] = pandas.read_csv(
                ...
                names=[
                    "id",
                    "n",
                    *(f"node_{node_index}" for node_index in range(1, 10)),
                ],
            )
        ...
        if num_land_boundaries > 0:
            for _ in range(num_land_boundaries):
                num_boundary_nodes, boundary_type = file.readline().split(maxsplit=2)[
                    :2
                ]
