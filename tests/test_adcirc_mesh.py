...
def test_triangles_only(nodes, elements):
    mesh = AdcircMesh(
        nodes,
        {id: geom for geom in elements.values() if len(geom) == 3},
    )
...
def test_quads_only(nodes, elements):
    mesh = AdcircMesh(
        nodes,
        {id: geom for geom in elements.values() if len(geom) == 4},
    )
...
def test_write(nodes, elements):
    ...
    with pytest.raises(ValueError):
        mesh.write(
            output_directory / "test_AdcircMesh.txt", format="txt", overwrite=True
        )
